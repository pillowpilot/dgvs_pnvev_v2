...
class CreateCasosChagasCronico extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement("CREATE OR REPLACE
            ALGORITHM = UNDEFINED VIEW `dgvsops`.`v_pnvev_casos_chagas_cronico` AS
            select
                11 AS `EnfermedadId`,
                (select name from dgvsops.pnvev_disease_v2s where id = 11) AS `TipoEnfermedad`,
                `ff`.`Sexo` AS `Sexo`,
                `ff`.`Edad` AS `Edad`,
                (case
    ...
    {
        DB::statement('DROP VIEW IF EXISTS `v_pnvev_casos_chagas_cronico`');
    }
}
